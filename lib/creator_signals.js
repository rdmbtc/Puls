...
export function registerCreatorSignals(app, deps) {
  ...
  app.get('/api/signals', authenticateUser, async (req, res) => {
    try {
  ...
  app.get('/api/signals/:id', authenticateUser, async (req, res) => {
    try {
  ...
  app.post('/api/signals/:id/unlock', authenticateUser, requireVerifiedUser, strictLimiter, async (req, res) => {
    try {
      ...
      // Resolve creator payout address from their wallet.
      const creatorWalletId = await getWalletId(signal.creator_user_id);
  ...
  app.get('/api/signals/:id/analytics', authenticateUser, async (req, res) => {
    try {
      const me = req.query.userId || null;
      const { data: row, error } = await supabase
        .from('creator_signals').select('*').eq('id', req.params.id).maybeSingle();
