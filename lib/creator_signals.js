...
export function registerCreatorSignals(app, deps) {
  ...
  function publicSignal(row, { withThesis = false, isOwner = false } = {}) {
    ...
    base.marketLink = base.marketSlug ? `${APP_BASE_URL}/m/${base.marketSlug}` : null;
    // Researched sources are public (proof the thesis is grounded, not the thesis itself).
    base.sources = Array.isArray(row.sources)
      ? row.sources
          .filter((x) => x && x.url)
          .slice(0, 4)
          .map((x) => ({ title: x.title || x.source || x.url, url: x.url, source: x.source || null }))
      : [];
    return base;
  }
