...
export function registerSwarm(app, deps) {
  ...
  async function ensureAgent(cfg) {
    ...
    // + its first trades. Graceful when the treasury is empty (one log, no spam).
    if (!s.ensured && balance < 0.3 && walletClient && adminAccount) {
      try {
        const treasury = await getTreasuryUsdcBalance();
        if (treasury != null && treasury >= BOOTSTRAP_USDC + 1) {
          await walletClient.writeContract({
            address: USDC, abi: usdcTransferAbi, functionName: 'transfer',
            args: [info.address, BigInt(Math.round(BOOTSTRAP_USDC * 1_000_000))],
          });
          await new Promise(r => setTimeout(r, 3000));
          info = await getWalletInfo(walletId);
          balance = parseFloat(info.usdcBalance) || 0;
          console.log(`[swarm:${cfg.key}] bootstrap-funded ${BOOTSTRAP_USDC} USDC`);
        } else {
          console.log(`[swarm:${cfg.key}] treasury too low to bootstrap (need ${BOOTSTRAP_USDC + 1}); will retry later`);
        }
