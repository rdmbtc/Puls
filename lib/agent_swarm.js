...
const DEFAULT_ROSTER = [
  ...
  },
  {
    ...
    category: 'worldcup',
    persona: 'a football analyst for the 2026 FIFA World Cup who turns live Polymarket odds + form/news into sharp, falsifiable calls with clear invalidation.',
  },
];
...
export function registerSwarm(app, deps) {
  ...
  async function worldCupMarkets() {
    ...
    const out = [];
    try {
      const r = await fetch('https://gamma-api.polymarket.com/events?slug=world-cup-winner', { headers: { Accept: 'application/json' } });
      if (r.ok) {
        const events = await r.json();
        const mkts = events?.[0]?.markets || [];
        for (const m of mkts) {
    ...
    if (out.length) _wcCache = { at: Date.now(), markets: out };
    return out;
  }
  ...
  async function runCreator(cfg) {
    ...
    if (cfg.category === 'worldcup') {
      ...
      // research the team on the open web, and let the LLM write the call.
      const wc = await worldCupMarkets();
      if (!wc.length) { console.log(`[swarm:${cfg.key}] no WC markets available`); return; }
      // Pick among the contenders (top ~12 by odds) so signals vary + stay real.
      const cand = wc.slice(0, 12);
      const m = cand[Math.floor(Math.random() * cand.length)];
      const yesPct = Math.round(m.yesPct * 100);
      // A "YES they win it all" call is only sane for genuine contenders; for
      // mid-table sides the falsifiable call is NO (won't win the cup).
      const stance = m.yesPct >= 0.10 ? 'YES' : 'NO';
      try {
        const res = await researchQuestion(`${m.team} 2026 FIFA World Cup chances form squad`, 3);
        researchBrief = res?.brief || '';
      } catch (_) {}
      pick = {
        t: `${m.team} at the 2026 World Cup`,
        q: m.question,
        s: stance,
        c: stance === 'YES' ? Math.min(0.7, m.yesPct + 0.05) : Math.min(0.8, 1 - m.yesPct),
