...
export function registerAgentBond(app, deps) {
  ...
  // SETTLE pass: RESOLVED-MARKET-DRIVEN so it never starves. (The old version
  // grabbed an arbitrary 25 ACTIVE bonds and only settled if THOSE happened to be
  // on resolved markets — with a large active backlog it kept re-checking the same
  // unresolved ones, so slashed/returned never moved.) Now: find resolved markets
  // first, then settle the active bonds linked to them.
