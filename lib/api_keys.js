...
export function registerApiKeys(app, deps) {
  ...
  app.get('/api/keys', authenticateUser, requireVerifiedUser, async (req, res) => {
    try {
      const userId = req.query.userId; // forced to the verified id
      const { data, error } = await supabase
        .from('api_keys')
      ...
      console.error('[api_keys] list error:', e.message);
      res.status(500).json({ error: e.message });
    }
  });
